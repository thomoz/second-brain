...
- Superinvestor Filings Scanner: investments/superinvestor-filings/ — daily VPS scan of tracked concentrated-value investors' fast-disclosure filings, grouped WhatsApp digest, advisor-notes only. US EDGAR leg (13D/G + Form 3/4/5, `second-brain-superinvestor-edgar.timer` 02:35 UTC) + India BSE SEBI SAST Reg 29 leg (`second-brain-superinvestor-sast.timer` 12:30 UTC). Run via `scripts/invoke_investments.ps1 -Package superinvestor-filings -Command "scan"` (`--edgar-only` / `--india-only` to restrict). See investments/superinvestor-filings/INDIA-LEG-FINDINGS.md for the BSE feed details.
