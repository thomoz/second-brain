...
- "leave a handoff for next local session" / "message for next prime" / "leave a message for local computer" → `Memory/whatsapp-handoff-messages-for-local-session.md` → `## Active handoffs`
...
| "handoff" / "next prime" / "local computer" | `Memory/whatsapp-handoff-messages-for-local-session.md` → under `## Active handoffs` |
