---
title: Finances
type: profile
category: finances
created: 2026-06-17
updated: 2026-06-17
---
# Finances

_Built through "Ask Me Questions" sessions. Append with dates — history matters._

## Full Financial Picture
_(not yet populated — run "ask me questions" to build this)_

## Income Sources
_(not yet populated)_

## Financial Goals
_(not yet populated)_

## Constraints
_(not yet populated)_

## 2026-06-26 Update

### Concentration And Currency Risk
- Feels fear around relying too heavily on SongbookDB as an income engine
- Worries that if SongbookDB fails, there may not be enough alternative income to compensate
- Sees diversification across the other businesses as a hedge against that risk
- Also worries about SongbookDB's exposure to USD revenue
- Concern: if the US dollar weakens sharply or collapses against AUD, SongbookDB income would become less valuable in local terms

### Live Work As Income Hedge
- Does not feel comfortable letting go of the live businesses
- Reason 1: the live work currently pays more than SongbookDB
- Reason 2: giving up stronger current income would feel unwise unless SongbookDB became materially more lucrative
- Current threshold thinking: only consider shutting down live work if SongbookDB grew enough that retirement within a couple of years became realistic

### SongbookDB Income Threshold
- Current after-tax income from SongbookDB is roughly $500 per week
- To feel comfortable backing it completely, it would need to earn about 20 times that level
- Implied comfort threshold is roughly $10,000 per week after tax

## 2026-06-17 Update

### Current Position (age ~52)
- Cash savings: ~$15,000
- Superannuation: ~$40,000
- Small share portfolio (see investment-strategy topic page)
- Actively saving — describes current savings rate as "decent"

### Savings Strategy
- Building cash buffer for two reasons: (1) protection if share market downturn reduces entertainment industry work, (2) dry powder to buy discount stocks if market crashes

### Retirement Math (self-calculated)
- Target retirement age: 67
- Target retirement income: ~$100,000/year
- Assumed index fund return: ~10%/year
- Calculated required savings rate to hit target: ~$1,000/week every week until 67
- Current super position is a significant concern — $40k at 52 is well behind typical trajectory

### Key Constraint
- Time poverty is the primary bottleneck — show hosting + travel + promotion consumes most available hours, limiting ability to build other income streams or invest time in higher-leverage activities

## 2026-06-24 Update

### Retirement Priority
- Retirement target is at least $1.5 million by age 67
- Almost 52 now
- Current super is about $40,000
- Current cash savings are about $15,000, built over the last 3 months
- Sees the retirement position as urgent and underbuilt
- Main financial priority is increasing income and investing capacity to improve retirement outcomes

### 12-Month Financial Security
- Would feel properly secure with $20,000 saved as an emergency fund
- Also wants $50,000 ready to invest into the stock market at discounted rates
