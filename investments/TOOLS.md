...
| <a id="goat-dma-breakout-scan"></a>[↑](#daily-read) **Goat DMA Breakout Scan** | Broad discovery scan across the combined S&P 500 + ASX 200 universe for names whose close just crossed ABOVE their 150-day or 200-day MA with that MA itself sloping up (checked independently, no base pattern, no rising-sector filter), with ethical filter, liquidity/market-cap floor, and fundamentals survival-context insolvency suppression. Stages fresh finds into `goat_pending_candidates` (`source="goat_dma_breakout_scan"`) and pushes new ones to WhatsApp; zero-candidate days are silent. | VPS systemd (`second-brain-goat-dma-breakout-scan.timer`) | Daily, 22:55 UTC (~08:55 AEST / 09:55 AEDT) | `investments/goat/dma-breakout-candidates-pending-review.md` |
...
| <a id="goat-hated-industries-scan"></a>[↑](#daily-read) **Goat Hated Industries Scan** | Contrarian counterpart to industry rotation ranking — a severity gate (bottom-5 of the 39 industry ETFs, underperforming SPY on both a 3mo and 6mo window, ≥25% drawdown from 52-week high, not already reversing) followed by an LLM web-search narrative read (industry-wide vs. concentrated in one/two names, plus the ETF's top holdings) and a fundamentals-divergence check (did revenue/earnings actually deteriorate, or just the price?) on the flagged industry's largest constituents. Industries only, no ticker staging; pushes new ones to WhatsApp, zero-candidate days are silent. | VPS systemd (`second-brain-goat-hated-industries-scan.timer`) | Daily, 23:50 UTC (~09:50 AEST / 10:50 AEDT) | `investments/goat/hated-industries-report.md` |
...
| **Goat hated-industries scan (on-demand)** | Same severity gate + narrative + fundamentals-divergence check, right now | `-Package goat -Command "scan-hated-industries"` | `investments/goat/hated-industries-report.md` |
