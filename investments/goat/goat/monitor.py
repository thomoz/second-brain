...
def _today_sydney() -> str:
    ...
    (Goat runs on the VPS, whose system clock is UTC)."""
...
def render_report(result: dict[str, Any]) -> str:
    lines = [
        "# Goat Monitor Report",
        "",
        "What this is: whether any holding/watchlist ticker has broken its "
        "150-day trend (exit-rule alerts). Sector-strength ranking from this "
        "same run is in sector-ranking.md, not this file.",
        "",
...
def render_industry_ranking_report(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "Auto-generated by Goat Monitor -- overwritten every run. Relative "
        f"performance over the trailing {config.GOAT_INDUSTRY_RANK_WINDOW_TRADING_DAYS} "
        "trading days (~6 months) across the industries below that have a "
        "dedicated, currently-trading ETF. Finer-grained sibling to "
    ...
    covered = [r for r in ranking if r["return_pct"] is not None]
    for row in covered[:5]:
        lines.append(f"| {row['rank']} | {row['ticker']} | {row['industry_label']} | {row['return_pct']:+.1f}% |")
    ...
    lines += [
        "",
        "## Full Ranking",
        f"All {len(ranking)} covered industries.",
        "",
    ...
    lines += [
        ...
        "No dedicated, currently-trading ETF found for these Finviz industries -- "
        "never silently substituted with a broader/looser proxy.",
        "",
    ]
    if not_covered:
        lines += [f"- {name}" for name in not_covered]
    else:
        lines.append("None.")
