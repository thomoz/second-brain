...
# 2026-09-20: first to drop every moving-average-relative gate (reclaim of the
# 50-day MA, 150-day MA dip tolerance, 150-day MA slope), then to drop the
# breakout requirement entirely -- per Shaun, a stock does not need to have
# broken out of its base yet to be a candidate (he'd rather see it before it
# has); only the base itself is evaluated now. See heartbeat.py's module
# docstring. All measures below are direct and recent-window -- no
# trailing-year self-percentile anchor. Price series is close-only (no
# intraday high/low), so range is a close-range.
